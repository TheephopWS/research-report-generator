...
def check_openalex(title: str):
    # OpenAlex prefers a "mailto" to put you in their "polite" pool
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if results:
                candidate = results[0].get("title")
                # Reuse your similarity logic
                if title_similarity(title.lower(), candidate.lower()) > 0.75:
                    return True, candidate
    except Exception:
        pass
...
def validate_citations(references: list[str], delay: float = 1.0) -> dict:
    ...
    for i, ref in enumerate(references):
        ...
        ss_found, cr_found, oa_found = False, False, False

        oa_found, oa_title = check_openalex(title)
        time.sleep(delay)
        ...
        if not ss_found and not oa_found:
            ss_found, ss_title = check_semantic_scholar(title)
            time.sleep(delay)

        is_valid = ss_found or cr_found or oa_found
        matched = ss_title if ss_found else (cr_title if cr_found else (oa_title if oa_found else ""))
        if ss_found and cr_found and oa_found:
            source = "all"
        elif ss_found:
...
def evaluate_all_contexts(contexts_dir: str, delay: float = 1.0) -> dict:
    ...
    for context in contexts:
        ...
        if context['references']:
            ...
            # Collect invalid citations
            for result in file_results['details']:
                if not result.is_valid:
                    all_results['invalid_citations'].append({
                        'file': context['file'],
    ...
    print(f"Total Invalid Citations: {len(all_results['invalid_citations'])}")
    print(f"{'='*60}\n")
    
    return all_results


def save_invalid_citations(results: dict, output_file: str) -> None:
    """
    Saves all invalid citations to a JSON file.
    
    Args:
        results: Dictionary returned from evaluate_all_contexts()
        output_file: Path to output JSON file
    """
    invalid_citations = results.get('invalid_citations', [])
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            'total_invalid': len(invalid_citations),
            'invalid_citations': invalid_citations
        }, f, indent=2, ensure_ascii=False)
    
    print(f"Invalid citations saved to: {output_file}")
    print(f"Total invalid citations: {len(invalid_citations)}\n")
...
if __name__ == "__main__":
    ...
    # Save invalid citations to file
    if results:
        output_file = os.path.join(script_dir, 'invalid_citations.json')
        save_invalid_citations(results, output_file)